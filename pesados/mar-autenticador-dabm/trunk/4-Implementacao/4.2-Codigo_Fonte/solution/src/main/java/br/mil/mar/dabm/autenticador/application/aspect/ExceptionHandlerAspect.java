package br.mil.mar.dabm.autenticador.application.aspect;
...
public class ExceptionHandlerAspect
{
	...
	public void handleException( JoinPoint joinPoint, javax.validation.ConstraintViolationException exception )
	{
		...
		for ( ConstraintViolation<?> constraint : exception.getConstraintViolations() )
		{
			...
			{
				message.append( "\nThe field " + constraint.getPropertyPath() + " must be set." );
			}
			else
			{
	...
	{
		// Caso a excecao ja tenha sido interceptada por outro Aspecto deve ser ignorada
		...
		if ( exception.getCause() instanceof ConstraintViolationException )
		{
			...
			String key;
			// Verifica o codigo do erro gerado pelo PostgreSQL
			switch ( cause.getSQLState() )
			{
				case "23503": // foreign_key_violation
				{
					key = message.substring( message.indexOf( '"' ) + 1, message.indexOf( '.' ) - 1 );
					throw new DataIntegrityViolationException( "Não foi possível realizar a operação pois este registro é referenciado em " + key + "." );
				}
				case "23505": // unique_violation
				{
					...
					if ( !key.equals( "login" ))
					{
						throw new DataIntegrityViolationException( "O campo " + key + " informado já existe. Altere e tente novamente." );
					}
					else
					{
						throw new DataIntegrityViolationException( "O campo NIP informado já existe. Altere e tente novamente." );
					}
				}
				...
				default:
				{
					throw new DataIntegrityViolationException( "Não foi possível realizar a operação pois ocorreu um problema de integridade (código " + cause.getSQLState() + "). Verifique e tente novamente." );
				}
			}
		}
	...
	// ---------
	/**
	 * Trata excecao de acesso negado
	 * 
	 * @param joinPoint
	 * @param exception
	 */
